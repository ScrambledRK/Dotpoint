...
import at.dotpoint.math.axis.AxisRotationOrder;
...
class MathQuaternion 
{
	 ...
	 * @return			provided `output` quaternion or a new one with the computational result
	 */
	public static function setEuler( euler:IVector3, ?order:AxisRotationOrder, ?output:IQuaternion ):IQuaternion
	{
		if( output == null )
			output = new Quaternion();

		if( order == null )
			order = AxisRotationOrder.YXZ;
