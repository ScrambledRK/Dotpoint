...
/**
 * builder to create memory-tight MeshData without knowing its signature (memory size) beforehand.
 * you can add/queue vertices, their data and faces as you wish and build once in the end.
 * 
 * @author RK
 */
class MeshBuilder
{
	...
	{
		this.lookup = new MeshIndexLookup( this.factory.getTypesSize() );		

		this.vertex = new Array<IVertex>();
		this.face = new Array<IFace>();

		this.entriesData = new Vector<Int>( this.factory.getTypesSize() );
	...
	public function buildFace():Bool
	{
		...
		//
		var type:Int = FaceType.getType( this.queueV.length );

		if ( type == -1 )
			throw "cannot add " + this.queueV.length + " vertices as no valid face type is available";

		//
		var face:IFace = this.factory.createFace();
		face.type = type;
