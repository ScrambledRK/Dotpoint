...
class BoundingsSingleTest
{
	...
	public function construct():Void
	{
		...
		this.entity.boundings.changePolicy = BoundingChangePolicy.CHANGE_MODELBOUNDS;
	}
	...
	private function testLocalSingle():Void
	{
		...
		Assert.areEqual( 40, model.width  );
		Assert.areEqual( 15, model.height );
		Assert.areEqual( 90, model.length );
		...
		Assert.isTrue( Vector3.isEqual( model.min, new Vector3(  0,  0,  0 ) ) );
		Assert.isTrue( Vector3.isEqual( model.max, new Vector3( 40, 15, 90 ) ) );
